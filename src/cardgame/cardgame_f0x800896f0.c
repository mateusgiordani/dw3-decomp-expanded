// CARDGAME:0x800896f0 (size 364, 0x16c)
// PAL: reference/extracted/pro/cardgame.bin base 0x80082cb0 file-off 0x6a40
// Framed function from boundary sweep symbols/function_labels.csv
// (CARDGAME_F0x800896f0, 364 B, PAL prologue + jr ra).
// Prologue 27bdffd8 addiu sp,-0x28 ; sw s0,0x18(sp) ; move s0,a0 ;
//   sw s1,0x1c(sp) ; move s1,a1 ; sw ra,0x24(sp) (+ sw s2 in jalr delay slot).
// Epilogue lw ra,0x24(sp) ; lw s2,0x20(sp) ; lw s1,0x1c(sp) ; lw s0,0x18(sp) ;
//   jr ra + addiu sp,+0x28 at 0x80089844-0x80089858.
// Next function CARDGAME:0x8008985c at +0x16c confirms size 0x16c;
// prev CARDGAME:0x80089580 size 368 ends at 0x800896f0 exactly.
// Ghidra program CARDGAME (project ddw3-pal-sles-03936) read-only, no mutation:
//   disasm 0x800896f0 (~89 insns incl. epilogue) word-equal to PAL bytes;
//   decompile 0x800896f0 -> void (int,int): (*DAT_80055c48)(0x4001b), two
//   indirect calls via *(param_2+0xf0c) with kinds 5 then 1, flag byte at
//   slot+0x150 bit0 clear/set, halfword at slot+0x12e = 0/1, index at
//   param_1+0x43c toggled ^1 between the calls (hypothesis only; C below
//   written from disassembly).
//   x-ref to 0x800896f0: 1 caller, CARDGAME_F0x80089974 @ 0x80089bcc
//   (jal 0x800896f0, delay move a1,s5; guarded by state machine case 2).
//   x-ref from: READ 0x80055c48 (EXE-resident vector), DATA 0x800a58d4
//   (CARDGAME pair table), stack/outgoing-arg writes only.
...
// Match notes: attempt 1 (named CardPair896f0 *t temp + direct
// (char*)ctx+idx*76 slot expressions) mismatched 30 bytes: table address
// kept in v1 (addu $3,$2,$18) instead of v0, and slot addu emitted
// offset-first (addu v0,v0,s1) instead of base-first (addu v0,s1,v0).
// Attempt 2 (this file): inline indexed pair accesses so CSE keeps the table
// address in v0 for both word loads, plus explicit int off temp so the
// ctx+off addition keeps base-first order (same idiom as the sibling
