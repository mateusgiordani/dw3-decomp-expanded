// CARDGAME:0x8009d6e0 -- PAL body 540 bytes; jump table 28 bytes at 0x800837a0.
// PAL: reference/extracted/pro/cardgame.bin base 0x80082cb0 file-off 0x1aa30
// Framed function: prologue addiu sp,-0x18 ; sw s0,0x10(sp) ; move s0,a0 ;
// lui v0,0x8004 ; lw v0,0x4f4c(v0) ; sw ra,0x14(sp) ; jalr v0 ; li a0,0x7a4 (delay).
...
// every word; bin words match Ghidra bytes (first 0x27bdffe8, jr ra 0x03e00008
// at 0x8009d8f4, delay 0x27bd0018 at 0x8009d8f8). No Ghidra state change.
...
// Callees: one indirect jalr via EXE slot DAT_80044f4c (allocator idiom, same as
// sibling CARDGAME:0x800973e4) with size 0x7a4; result kept at s+0x2f0.
// Data: jump table at 0x800837a0 (7 words, bin-verified 0x8009d7b4..0x8009d7e4),
// halfword cost table at 0x800a5c2c (bin-verified).
...
// (K: 1->0x190, 2->0x12c, 3->0x1f4, 4->0xc8, 5->0x258, 6->0x64, 7->0x2bc;
// b3==0 or >7 stores nothing). Loop2 scans 27 halfwords at rec+0xa0:
// zero halfword writes the 15-byte default row at s+k and exits; else
// s[0x400+k]=(byte)(half-1). Loop3 copies 40 rec halfwords (stride 4) to
// s[0x248+k*2] as (h&0xfff)-1. cardgame.s is GUIDE only, never copied.
// Exact with pinned PsyQ GCC 2.8.1 / ASPSX 2.79, O2 base (strength reduction on).
// Indexed loop-1 accesses let the compiler initialize its derived cursors after
// the jump-table address. In loop 2, k remains live through the zero-byte address;
// its transition from row index to byte offset preserves PAL's temporary copy.
